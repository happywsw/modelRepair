...
public class BuildInitialPopulation {
	...
	private GeneticMiningMatrices geneticMatrices = null;
...
//                System.out.println("individual number " + i + " "+ j + " = " + population[i].getOutputSet(j).toString());
				if (geneticMatrices.getStartMatrix().get(j) > 0) {
				...
				if (geneticMatrices.getEndMatrix().get(j) > 0) {
...
//           System.out.println("individual number " + i + " start task  = " + population[i].getStartTask() + " and end task = " + population[i].getEndTask());
			...
			geneticMatrices.rebuildAllMatrices();
		}
	...
	private HNSet buildInputSet(int index) {
		...
		return MethodsOverIndividuals.buildSet(geneticMatrices.getStartMatrix().get(index),
				geneticMatrices.getCausalMatrix().viewColumn(index), generator);
	}

	private HNSet buildOutputSet(int index) {
		return MethodsOverIndividuals.buildSet(geneticMatrices.getEndMatrix().get(index),
				geneticMatrices.getCausalMatrix().viewRow(index), generator);
	}
