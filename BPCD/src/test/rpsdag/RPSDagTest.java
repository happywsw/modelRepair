...
import graph.rpsdag.RPSDag;
import graph.rpsdag.db.DBHelperDb;
import graph.rpsdag.db.DBHelperDb.Stmts;
import mtree.index.*;
import mtree.index.Timer;
	...
	public static void main(String[] args) throws Exception {
		...
		if (a[0].equals("-size")) {
			...
			try {
				...
				System.out.println("Invalid input: size " + a[1] + ". Using default, 4 for minimal clone size.");
			}
		}
			...
			for (int k = 0; k < listOfFiles.length; k++) {							
				if (listOfFiles[i].isFile()) {
					...
					try {						
						...
						epcmodel.cleanEPC();
						engine.addProcessModel(new EPCHelper(epcmodel, filepath), filename);
					} catch (Throwable e) {
						//----------------------------------------------------------
		...
		try {
			...
			while(rs.next()) {
				...
				System.out.println();
				System.out.print(cloneid + "\t\t" + rs.getLong(2)+ "\t\t" + rs.getLong(3) + "\t");
				...
				for (String m : models) {
					...
					if(!printed) {
						...
						epcmodel.cleanEPC();
						printed = engine.printProcessModelClone(new EPCHelper(epcmodel), cloneid);
					}
				}
