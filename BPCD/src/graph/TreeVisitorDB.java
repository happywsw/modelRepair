...
import graph.rpsdag.db.DBHelperDb;
import graph.util.SortedListPermutationGenerator;
...
public class TreeVisitorDB implements Visitor {
	...
	public TreeVisitorDB(Helper helper, DBHelperDb DBHelperDb, boolean insertingMode) {
		this.helper = helper;
		this.DBHelperDb = DBHelperDb;
		this.insertingMode = insertingMode;
	}
	...
	public String visitPNode(Graph graph, Set<Edge> edges,
		...
		for (Integer v: vertices) {
			sortedLabels.add(new QueueEntry(v, getLabelForQueueEntry(v, graph, vertices), 
					DBHelperDb.getIndexForHash(graph.getLabel(v).replaceAll("\\s+", "")).getFirst(),
					DBHelperDb.getIndexForHash(graph.getLabel(v).replaceAll("\\s+", "")).getSecond()));
		}

		Collections.sort(sortedLabels);
		
		
		String[] buffer = getBestLabelHash(graph, edges, vertices, entry,
				exit, sortedLabels, children);
	...
	public String visitRootSNode(Graph graph, Set<Edge> edges,
		...
		// there can be more than one branch connected to the entry node (if model is disconnected)
	...
	public String visitSNode(Graph graph, Set<Edge> edges,
		...
		while (true) {
			...
			if (data != null) {
				Integer ndx = data.getFirst();
				if (!DBHelperDb.isGateway(ndx) && !DBHelperDb.isEntryOrExitNode(ndx)) {
					nodesize += data.getSecond();
				} else {
					size--;
				}
		...
		if (insertingMode) {
			Integer id = DBHelperDb.addModelHash(hash, Math.min(1, size), children);
			DBHelperDb.addParentChildRelations(id, children);
		}
		foldComponent(graph, edges, vertices, entry, exit, hash);
	...
	private String getLabelForQueueEntry(Integer vertex, Graph graph, Set<Integer> vertices) {
		
		Set<Integer> parents = graph.getPredecessorsOfVertex(vertex);
		Set<Integer> children = graph.getSuccessorsOfVertex(vertex);
		// we have a gateway
		if ((parents.size() > 1 || children.size() > 1) && DBHelperDb.isGateway(graph.getLabel(vertex))) {
			StringBuffer buffer = new StringBuffer();
			...
			buffer.append(DBHelperDb.getIndexForHash(graph.getLabel(vertex).replaceAll("\\s+", "")).getFirst() + ".");
