...
public class RPSDag {
	...
	public void addProcessModel(Helper helper, String filename) throws Exception {
		addProcessModel(helper, false, filename);
	}

	public void addProcessModel(Helper helper, boolean print, String filename) throws Exception {
	...
	private void traverse(ExpRPST tree, Map<TreeNode, String> codeMap,
		...
		visitor.visitRootSNode(graphp, edges, vertices,
	...
	private void traverse(ExpRPST tree, Graph graph, TreeNode curr,
		...
		for (TreeNode child : curr.getChildNodes()) {
			...
			case S:
				...
				code = visitor.visitSNode(graph, cedges, cvertices, entry, exit, v.size());
				break;
			case P:
				v = child.getOriginalVertices();
				code = visitor.visitPNode(graph, cedges, cvertices, entry, exit, v.size());
				break;
			case Q:
				break;
			case R:
				v = child.getOriginalVertices();
				code = visitor.visitRNode(graph, cedges, cvertices, entry, exit, v.size());
				break;
			}
	...
	private boolean traverseClones(ExpRPST tree, Map<TreeNode, String> codeMap,
		...
		Collection<Integer> v = root.getOriginalVertices();
		boolean fileWritten =  traverseClones(tree, graphp, root, edges, vertices, visitor,
				codeMap, helper, nodeId, idOriginalVerticesMap);
		
		if (fileWritten) {
			return true;
		}
		
		String code = visitor.visitRootSNode(graphp, edges, vertices, tree.getEntry(root), tree.getExit(root), v.size());
		...
		// this is a substring
		if (("." + code + ".").indexOf("." + ConsoleDag.idToHashMap.get(nodeId) + ".") != -1) {
		...
		for (TreeNode child : curr.getChildNodes()) {
			if (child.getNodeType() == SPQRNodeType.Q) {
				if (!helper.isGateway(tree.getEntry(child))) {
					...
					idOriginalVerticesMap.put(dbHelper.getIndexForHash(code).getFirst(), v);
			...
			case S:
				...
				// this is a substring
				if (("." + code + ".").indexOf("." + ConsoleDag.idToHashMap.get(nodeId) + ".") != -1) {
					helper.serializeToFile(ConsoleDag.idToHashMap.get(nodeId), idOriginalVerticesMap, "" + nodeId);
					return true;
				}
				break;
			case P:
